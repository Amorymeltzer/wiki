...
my $multiReturn = JSON::MaybeXS->new->decode(read_text('t/file.json'))->{query};
my @users       = sort keys %{JSON::MaybeXS->new->decode(testFile('t/file.json'))};
...
sub testFile {
  ...
  my $pageReturn = JSON::MaybeXS->new->decode($fileJSON)->{query};
