...
mw.loader.load('//en.wikipedia.org/w/index.php?title=User:Amorymeltzer/pinfo.js&action=raw&ctype=text/javascript'); // [[User:Smith609/toolbox.js]], [[User:Amorymeltzer/pinfo.js]]
...
if (cfg.wgCanonicalNamespace === 'Special') {
	switch (cfg.wgCanonicalSpecialPageName) {
		case 'Contributions':
			...
			mw.loader.load('//en.wikipedia.org/w/index.php?title=User:Writ_Keeper/Scripts/massRollback.js&oldid=1371257788&action=raw&ctype=text/javascript'); // [[User:Writ Keeper/Scripts/massRollback.js]]
			...
			$(document).on('click', '.mass_revdel_on', function() {
				mw.loader.load('//en.wikipedia.org/w/index.php?title=User:Writ_Keeper/Scripts/massRevdel.js&oldid=1301300625&action=raw&ctype=text/javascript'); // [[User:Writ Keeper/Scripts/massRevdel.js]]
	...
	*/
	...
	if ((cfg.wgNamespaceNumber === 0) || (cfg.wgNamespaceNumber === 1)) {
		...
		mw.loader.using([
			...
			if (!$('li.new[id|=ca-nstab]').length) {
				mw.loader.load('//en.wikipedia.org/w/index.php?title=User:Evad37/rater/app.js&oldid=1367315678&action=raw&ctype=text/javascript');
			}
		});
	}
	...
	if (cfg.wgNamespaceNumber === 0) {
		...
		*/
		...
		mw.util.addPortletLink('p-cactions', '#', 'ew check', 'ca-ewchecker');
		...
		*/
		...
		if (cfg.wgNamespaceNumber === 4) {
			...
			// mw.loader.load('//en.wikipedia.org/w/index.php?title=User:MusikAnimal/responseHelper.js&oldid=1354954018&action=raw&ctype=text/javascript'); // placeholder
			...
			if (cfg.wgPageName === 'MediaWiki:Gadgets-definition') {
				...
				mw.loader.load('//en.wiktionary.org/w/index.php?title=User:Erutuon/scripts/gadgets-definition.js&oldid=82331001&action=raw&ctype=text/javascript');
			}
		}// END else if LOOP but remain in not articles

		/* WP and all talks */
		if ((cfg.wgNamespaceNumber === 4) || (cfg.wgNamespaceNumber % 2 === 1)) {
			mw.loader.load('//en.wikipedia.org/w/index.php?title=User:Pythoncoder/Scripts/voteSymbols.js&oldid=1350350783&action=raw&ctype=text/javascript'); // [[User:Ais523/votesymbols.js]], [[User:Pythoncoder/Scripts/voteSymbols.js]], [[User:Pythoncoder/Scripts/voteSymbols]]
...
if (mw.config.exists('wgRelevantUserName')) {
	// mw.loader.load('//en.wikipedia.org/w/index.php?title=User:PleaseStand/userinfo.js&oldid=1351866189&action=raw&ctype=text/javascript'); // placeholder
