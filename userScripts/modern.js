...
if (cfg.wgCanonicalNamespace === 'Special') {
	...
	*/
	/* Not Special */
	mw.loader.load('//en.wikipedia.org/w/index.php?title=User:Ale_jrb/Scripts/csdhelper.js&oldid=1241828968&action=raw&ctype=text/javascript'); // [[User:Ale jrb/Scripts]], [[User:Ale jrb/Scripts/csdhelper.js]]
	...
	if ((cfg.wgNamespaceNumber === 0) || (cfg.wgNamespaceNumber === 1)) {
		...
		mw.loader.using([
			...
			if (!$('li.new[id|=ca-nstab]').length) {
				mw.loader.load('//en.wikipedia.org/w/index.php?title=User:Evad37/rater/app.js&oldid=1249462961&action=raw&ctype=text/javascript');
			}
		});
	}
	...
	if (cfg.wgNamespaceNumber === 0) {
		...
		*/
		...
		if (cfg.wgNamespaceNumber === 4) {
			...
			// mw.loader.load('//en.wikipedia.org/w/index.php?title=User:MusikAnimal/responseHelper.js&oldid=1176115582&action=raw&ctype=text/javascript'); // placeholder
